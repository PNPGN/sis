...
if(isset($_POST['register'])){
    ...
    if(!isset($_POST['username']) || $_POST['username'] == ''){
        ...
        $errors[] = urlencode('Lietotājvārds ir obligāts');
        
    } elseif(strlen($_POST['username']) < 5){
        
        $errors[] = urlencode('Lietotājvārdam jābūt vairāk kā 5 simboliem');
    ...
    if(!isset($_POST['firstname']) || $_POST['firstname'] == ''){
        ...
        $errors[] = urlencode('Vārds obligāts');
        
    } elseif(!preg_match("/[a-zA-Z]/", $_POST['firstname'])){
        
        $errors[] = urlencode('Vārdam jāsastāv tikai no burtiem');
    ...
    if(!isset($_POST['lastname']) || $_POST['lastname'] == ''){
        ...
        $errors[] = urlencode('Uzvārds obligāts');
        
    } elseif(!preg_match("/[a-zA-Z]/", $_POST['lastname'])){
        
        $errors[] = urlencode('Uzvārdam jāsastāv tikai no burtiem');
    ...
    if(!isset($_POST['password']) || $_POST['password'] == ''){
        ...
        $errors[] = urlencode('Parole obligāta');
        
    } elseif(strlen($_POST['password']) < 8){
        
        $errors[] = urlencode('Parolei jābūt vairāk kā 8 simboliem');
    ...
    if(!isset($_POST['phone']) || $_POST['phone'] == ''){
        ...
        $errors[] = urlencode('Lūdzu ievadi savu telefona nr.');
        
    } elseif(!is_numeric($_POST['phone'])){
            
            $errors[] = urlencode('Nederīgs nr.');
    ...
    if(!isset($_POST['country-code']) || $_POST['country-code'] == ''){
        ...
        $errors[] = urlencode('Valsts kods');
        
    } elseif(!preg_match("/[+.0-9]/", $_POST['country-code'])){
            
            $errors[] = urlencode('Nederīgs valsts kods');
    ...
    if(!isset($_POST['title']) || $_POST['title'] == ''){
        ...
        $errors[] = urlencode('Izvēlieties uzrunu');
        
    } elseif(!preg_match("/[a-zA-Z]/",$_POST['title'])){
            
            $errors[] = urlencode('Nederīga uzruna');
    ...
    if($query->num_rows > 0){
        $errors[] = urlencode("Lietotājvārds ar šādiem datiem jau pastāvt");
    }
    ...
    if(!$errors){
        ...
        if($row == 1){
            ...
            $msg = urlencode("Apsveicu $lastname! Tu esi veiksmīgi "
                    . "reģistrējies");
                
            header('Location:thankyou.php?msg='.$msg);
            
            $subject = "Reģistrācija veiksmīga";
            $raw_message = "Sveiks $username!
                \t\tŠis paradzēts, lai informētu jūs, ka jūsu konts ir izveidots veiksmīgi un gaida administratora apstiprinājumu. Kad esat apstiprināts, jūs varat pieprasīt atvaļinājumu un spēt pārvaldīt savus atvaļinājumus.\n\t\t\tAr cieņu\n\t\t Online Atvaļinājumi";
    ...
    if(var_set($_POST['make-supervisor'])){
        ...
        if(is_string($_POST['make-supervisor'])){
            ...
            if($db_con->affected_rows == 1){
                ...
                $msg= urlencode("$name ir piešķirta vadītāja loma");
    ...
    if(var_set($_POST['assign-to'])){
        ...
        if(is_string($_POST['supervisor'])){
            ...
            $error[] = urlencode("Nederīgs vadītājs");
        }
        ...
        if(is_string($_POST['assign-to'])){
            ...
            $error[] = urlencode("Nederīga lietotāja izvēle");
        }
    }
    ...
    if(!$error){
        ...
        if($db_con->affected_rows == 1){
            ...
            $msg= urlencode("$name ir pievienots vadītājam:  $sup");
    ...
    if(!$error){
        ...
        if($db_con->affected_rows == 1){
           ...
           if($db_con->affected_rows == 1){
               ...
               $subject = "Atvaļinājums ir apstiprināts";
               
               $raw_message = "Apsveicu $firstname!\n\\Tavs Atvaļinājums ar ID:
                      $leave_id ir apstiprināts. Tagad tu vari izbaudīt savas brīvdienas 
                      \t\t\tVēlu Veiksmi";
               ...
               if(mail($to, $subject, $message, $from)){
                    ...
                    $msg= urlencode("Atvaļinājums ir apstiprināts");
               ...
               $msg= urlencode("Atvaļinājuma pieprasījums netika apstiprināts");
    ...
    if(var_set($_POST['staff_id'])){
        ...
        $err[] = urlencode("Nevar apstiprināt darbinieku. Mēģiniet vēlreiz!");
    }
    ...
    if(!$err){
        ...
        if($db_con->affected_rows == 1){
            ...
            $msg = urlencode("Darbinieks apstiprināts");
            redirect_user("admin.php?tab=2&msg=$msg");
        }else{
            $error = urlencode("Nevar apstiprināt darbinieku ".$db_con->error);
    ...
    if(var_set($_POST['date_joined'])){
        ...
        $errors[] = urlencode("Pievienošanās datumam jābūt pievienotam");
    }
    
    if(var_set($_POST['salary_level'])){
        $salary_level = strip_tags($_POST['salary_level']);
    }else{
        $errors[] = urlencode("Algai jābūt pievienotai");
    }
    ...
    if(var_set($_POST['staff_id'])){
        ...
        $errors[] = urlencode("Izvēlies darbinieku");
    }    
    ...
    if(!$errors){
        ...
        if($res->num_rows > 0){
            ...
            if($db_con->affected_rows == 1){
                $msg = urlencode("Darba apraksts veiksmīgi pievienots");
    ...
    if(var_set($_POST['annual_leave_days_allowed'])){
        ...
        $errors[] = urlencode("Ievadi dienu skaitu");
    }
    ...
    if(var_set($_POST['staff_level'])){
        ...
        $errors[] = urlencode("Izvēlies darbinieka līmeni");
    }
    ...
    if(!$error){
        ...
        if($db_con->affected_rows == 1){
            ...
            if($db_con->affected_rows == 1){
                ...
                $raw_message = "Sveiks $firstname!\r\n\tTavs Atvaļinājums ar ID 
                            $leave_id ir noraidīts. Par iemesliem lūdzu sazinies ar savu vadītāju.\r\n\r\n\t\t\t
                            Ļoti atvainojos!";
                
                $message = wordwrap($raw_message, 70, "\n\t");

                $from = "onlineatvalinajumi@gamil.com";

                if(mail($to, $subject, $message, $from)){
            
                     $msg = urlencode("Atvaļinājums noraidītss");
